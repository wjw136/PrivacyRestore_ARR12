...
from common_utils import inject_prompt, format_questions_legal, format_ans, lt_modulated_vector_add_new_direction, \
    lt_modulated_vector_add_new_direction_client, client_direction, lt_modulated_vector_add_new_direction_speed, \
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if config.use_inter:
            ...
            print(f"Parameters: evidences_len={self.evidences_len}, interHead_len={self.interHead_len}")
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if self.config.use_inter:
            ...
            for idx, evidence in enumerate(self.model.bank_evidences):
                symptom_dir = os.path.join(self.config.bank_intervention_dir, f"{self.model.evidences2id[evidence]}/")
    ...
    def forward_intervention_orpo(
        ...
        user_text = format_questions_legal(input['question_mask'])
        ...
        LM_loss = torch.tensor(0).to(device)


        # # LM loss based on seed evidence
    ...
    def base_infer_mc_score(
        ...
        input_prompt = format_questions_legal(question)
    ...
    def intervention_infer_mc_score(
        ...
        input_prompt = format_questions_legal(input['question_mask'])
        layers_to_intervene = []
        for flattened_idx in self.model.intervention_heads:
    ...
    def intervention_infer_mc_score_client(
        self,
        input = None,
        tokenizer = None,
        device = None,
        ...
        # client
        client_directions = client_direction(self, restore_evidences=input['restore_evidences'], re_evi_attention=re_evi_attention,
                                             device = device, dx_privacy=dx_privacy)
        intervene = partial(lt_modulated_vector_add_new_direction_client, llamaModel=self, restore_evidences=input['restore_evidences'], \
                                re_evi_attention = re_evi_attention, multiplier = multiplier, client_vectors=client_directions)
