...
def train():
    ...
    parser.add_argument("--score_mask", type=str2bool, default="false", help='local directory with model data')
    ...
    parser.add_argument("--max_len", type=int, default=256, help='local directory with model data')
        ...
        with torch.no_grad():
            for idx, data in enumerate(tqdm(evaluate_dataset)):
                ...
                intervention_gen_str= main_model.intervention_generate(data, tokenizer, device = device,
                                                                auxiliary_model = auxiliary_model, auxiliary_tokenizer=auxiliary_tokenizer, output_len = args.max_len,
                                                                dx_privacy = dx_privacy,
                                                                use_weight = args.use_weight)

                init_gen_str= main_model.base_generate(data['question_init'], tokenizer,device = device, output_len = args.max_len)
                
                if args.score_mask:
                    mask_gen_str= main_model.base_generate(data['question_mask'], tokenizer, device = device , output_len = args.max_len)
                ...
                print(f"[QUESTION]: {data['question_mask']}")
