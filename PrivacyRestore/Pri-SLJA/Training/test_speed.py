...
def train():
    ...
    with open(output_ans_path, 'w') as output_ans_file:
        with torch.no_grad():
            for idx, data in enumerate(tqdm(evaluate_dataset)):
                ...
                intervention_gen_str, output_len= main_model.intervention_generate_speed(data, tokenizer, device = device,
                                                                auxiliary_model = auxiliary_model, auxiliary_tokenizer=auxiliary_tokenizer, output_len = args.max_len,
                                                                dx_privacy = dx_privacy)

                ender.record()
                torch.cuda.synchronize()
                time = starter.elapsed_time(ender) ## 从 starter 到 ender 之间用时,单位为毫秒
                TP = 1000/(time/output_len)
    ...
    print(f"final time: {np.mean(times)}, final TP: {np.mean(TPs)}")
